...
WRITE_ROLES = ("quick-implementer", "implementer", "luna-escalation", "commit-pusher")
...
class OrchestrationTests(unittest.TestCase):
    ...
    def test_single_read_only_drift_fails(self) -> None:
        report = ro.build_role_evidence(
            stream_for("code-explorer"),
            digests(("a.txt",)),
            digests(("a.txt", "sneaky.txt")),
        )
        by_role = {item.role: item for item in report.roles}
        self.assertEqual("FAIL", by_role["code-explorer"].verdict)
        self.assertIn("READONLY_DRIFT", by_role["code-explorer"].reason_codes)
    ...
    def test_commit_pusher_requires_authorization(self) -> None:
        report = ro.build_role_evidence(stream_for("commit-pusher"), digests(()), digests(()))
        by_role = {item.role: item for item in report.roles}
        self.assertEqual("BLOCKED", by_role["commit-pusher"].verdict)
        self.assertIn("COMMIT_NOT_AUTHORIZED", by_role["commit-pusher"].reason_codes)

    def test_missing_role_surfaces_parse_reason(self) -> None:
        report = ro.build_role_evidence(
            stream_for("code-explorer"), digests(()), digests(())
        )
